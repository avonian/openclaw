#!/usr/bin/env python3
"""
Sync agent session messages to Graphiti knowledge graph.
...
SYNC_STATE_FILE = CONFIG_DIR / ".graphiti-sync-state.json"
...
def check_graphiti():
    ...
    try:
        req = urllib.request.Request(f"{GRAPHITI_URL}/search", method="POST",
...
def should_sync_message(content):
    ...
    skip_patterns = [
        "HEARTBEAT_OK", "HEARTBEAT", "NO_REPLY",
        "New session", "[message_id:",
    ]
...
def sync_sessions():
    ...
    for agent_id, sessions_dir in find_all_agent_sessions():
            ...
            try:
                with open(session_file) as f:
                    for line in f:
                        ...
                        speaker = "User" if role == "user" else agent_id.capitalize()

                        if send_to_graphiti(group_id, role_type, speaker, content, timestamp):
    ...
    print(f"Graphiti session sync: {synced_count} messages synced")
    return synced_count
